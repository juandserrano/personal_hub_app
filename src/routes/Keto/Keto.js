...
const Keto = () => {
    ...
    const updateUltimoPeso = async () => {
        const { data: { lastDate, lastWeek, lastWeight  } } = await axios.get('http://192.168.50.111:50000/api/keto')
    ...
    const updatePeso = async () => {
        ...
        const { data } = await axios.post('http://192.168.50.111:50000/api/keto',{ peso });
        console.log(data);
        if(data.error == false){
            updateUltimoPeso();
        }
        ...
        <Grid container justify='center' direction='row'>
            ...
            <Grid item>
                <Card className={classes.mainCard} raised>
                    ...
                    <br/>
                    <Grid container>
                        <Grid item xs={5}><Typography variant='body1'><strong>Semana:</strong></Typography></Grid><Grid item xs={7}><Typography variant='body1'>{semana}</Typography></Grid>
                        <Grid item xs={5}><Typography variant='body1'><strong>Fecha:</strong></Typography></Grid><Grid item xs={7}><Typography variant='body1'>{fecha}</Typography></Grid>
                        <Grid item xs={5}><Typography variant='body1'><strong>Peso:</strong></Typography></Grid>{pesoViejo != '' ? <Grid item xs={7}><Typography variant='body1'>{pesoViejo}kg</Typography></Grid> : null}
                    </Grid>
                    ...
                    <Grid container justify='center'>
                        <Grid item>
                            <Fab onClick={updatePeso} variant="extended" color="primary" aria-label="add" className={classes.margin}>
